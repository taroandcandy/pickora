...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  const isImageType = ['image/jpeg', 'image/png', 'image/gif'].includes(
    rawFile.type,
  )
...
const comfirm = async () => {
  ...
  let result: any = await reqAddOrUpdateTrademark(
    trademarkParams as UpdateTradeMarkData | AddTradeMarkData,
  )
  // 添加或修改品牌
  if (result.code === 200) {
    // 关闭对话框
...
const removeTradeMark = async (id: string) => {
  ...
  if (result.code === 200) {
    getHasTrademark(
      tradeMarkArr.value.length ? currentPage.value : currentPage.value - 1,
    )
    ElMessage({
      type: 'success',
      message: '删除品牌成功',

            ...
            <div v-show="scene === 1">
                ...
                <el-table border style="margin: 10px;" :data="SpuArr">
                    ...
                    <el-table-column width="300px" label="操作">
                        <template #="{ row, $index }">
                            <el-button style="width: 35px;height: 25px;" type="primary" size="default" icon="Plus"
                                title="添加SKU" @click="addSku(row)"></el-button>
                            <el-button style="width: 35px;height: 25px;" type="primary" size="default"
                                @click="updateSpu(row)" icon="Edit" title="修改SPU"></el-button>
                            <el-button style="width: 35px;height: 25px;" type="primary" size="default"
                                @click="viewSku(row.id)" icon="View" title="查看SKU列表"></el-button>
                            ...
                            <el-popconfirm :title="`确认删除${row.spuName}吗？`" width="200px" @confirm="deleteSpu(row.id)">
                                <template #reference>
                                    <el-button type="primary" icon="Delete" style="width: 35px;height: 25px;"
...
import type { SpuData, HasSpuResponseData, SpuRecordList } from '@/api/product/spu/type'
...
const deleteSpu = async(id:number) => {
    ...
    if(result.code === 200){
        ...
        getHasSPU(SpuArr.value.length > 1 ? currentPage.value : currentPage.value - 1)
    }else{

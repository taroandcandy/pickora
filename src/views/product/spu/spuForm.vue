        ...
        <el-form label-width="100px">
            ...
            <el-form-item label="SPU品牌">
                <el-select placeholder="请选择品牌" v-model="form.tmId">
                    <el-option v-for="(item, index) in attrArr" :key="item.id" :label="item.tmName"
                        :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="SPU描述">
                <el-input placeholder="请你输入描述" type="textarea" v-model="form.description"></el-input>
            </el-form-item>
            <el-form-item label="SPU照片">
                <ImageUploader v-model:imageList="form.spuImageList" action="/api/myApi/product/spu/uploadImage"
                    :limit="5" />
            </el-form-item>

            <el-form-item label="SPU销售属性" >
                <!-- 选择销售属性 + 添加 -->
                <el-select v-model="currentSaleName" :placeholder="`当前还有${unselectedAttrCount}个销售属性未设置`"
                    style="width: 240px">
                    <el-option v-for="item in availableAttrs" :key="item" :label="item" :value="item" />
                </el-select>
                <el-button type="primary" style="margin-left: 10px" @click="addSaleAttr">+ 添加属性</el-button>
                ...
                <el-table :data="saleEditors" border>
                        ...
                        <template #default="{ row }">
                            <div style="display: flex; flex-wrap: wrap; gap: 4px">
                                ...
                                <template v-for="(item, idx) in row.attrValues" :key="idx">
                                    <!-- 这里时可关闭的tag -->
                                    <el-tag v-if="!item.editing" closable @close="removeAttrValue(row, idx)"
                                        @click="editAttrValue(row, idx)">
                                        {{ item.value }}
                                    </el-tag>
                                    <el-input v-else v-model="item.value" @blur="saveAttrValue(row, idx)"
                                        style="width: auto; min-width: 30px" />
                                </template>
                                <!-- 每行添加按钮 -->
                                <el-button icon="Plus" type="primary" @click="addAttrValue(row)"
                    ...
                    <el-table-column label="操作" width="80px">
                        <template #="{ row, $index }">
                            <!-- pop框提示是否确认删除 -->
                            <el-popconfirm :title="`确认删除${row.value}吗？`" width="200px" @confirm="removeAttrRow($index)">
                                <template #reference>
...
import type { SpuAttrResponseData, SpuAttrValue, SpuRecordList, SpuPoster, SpuData, SpuSaleAttr, EditableSpuPoster } from '@/api/product/spu/type'//attrValueNew
...
function addSaleAttr() {
    ...
    const exists = saleEditors.value.some(e => e.attrName === currentSaleName.value)
    if (exists) {
...
function saveAttrValue(row: EditableSpuPoster, idx: number) {
    ...
    const isDuplicate = row.attrValues.some((v, i) => v.value === val && i !== idx)
    if (isDuplicate) {
        row.attrValues.splice(idx, 1)
...
const availableAttrs = computed(() => {
    return spuSaleList.value.filter(attr => !saleEditors.value.some(e => e.attrName === attr))
})

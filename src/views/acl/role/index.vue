    ...
    <div class="user-container">
        ...
        <el-card shadow="always" :body-style="{ padding: '20px' }" style="margin-bottom: 10px;">
            <el-button type="primary" size="large" @click="handleAdd" style="margin-right: 5px;padding: 5px 10px;"
                icon="Plus">添加角色</el-button>
            <el-table :data="roleList" border stripe style="margin-top: 10px;">
                <el-table-column type="index" width="60px" label="序号" />
                <el-table-column label="ID" width="60px" prop="id">
                </el-table-column>
                <el-table-column label="角色名称" width="100px" prop="roleName" show-overflow-tooltip>
                </el-table-column>
                ...
                <el-table-column label="操作" width="300px">
                    <template #="{ row, $index }">
                        <el-button type="primary" size="default" @click="handlePermission(row)"
                            icon="User">分配权限</el-button>
        ...
        <!-- 分配权限抽屉 -->
        <el-drawer title="分配权限" v-model="isDrawer" direction="rtl" size="30%" :destroy-on-close="true"
...
import type { RoleResponseData, RoleResponseValue, ResponseData, AddOrUpdateRole, MenuList, MenuResponseData, MenuData } from '@/api/acl/roleManage/type'
import { reqDeleteRole, reqAddOrUpdateRole, reqGetPermission, reqUpdatePermission } from '@/api/acl/roleManage'
...
const rules = {
    roleName: [{ required: true, trigger: 'change', validator: validateRoleName }],
}
// 弹窗关闭
const cancel = () => {
    ...
    try {
        ...
        if (result.code === 200) {
            if (form.id) {
                ...
                getHasRoleList(currentPage.value = Math.ceil((total.value + 1) / pageSize.value))
            }
            isDialog.value = false
        }
    } catch (error) {
...
const collectOptimizedNames = (nodes: MenuData[], selectedIds: number[]): string[] => {
    const result: string[] = []
...
// 最终绑定的事件函数
const handleCheck = ( _: MenuData[],checkedKeys: { checkedKeys: number[]; halfCheckedKeys: number[] }) => {
...
const submitPermissionForm = async() => {
    try {
        let result: ResponseData = await reqUpdatePermission(selectedId.value,selectedNames.value)
        if(result.code === 200){
...
const getHasRoleList = async (pager = 1) => {
    try {
        ...
        let result: RoleResponseData = await reqGetRoleList1(currentPage.value, pageSize.value)
        if (result.code === 200) {

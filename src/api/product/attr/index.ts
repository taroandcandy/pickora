...
// 二级分类接口
export const reqC2 = (C1_id: string | number) =>
  request.get<any, categoryResponseData>(`${API.C2_URL}/${C1_id}`)
// 三级分类接口
export const reqC3 = (C2_id: string | number) =>
  request.get<any, categoryResponseData>(`${API.C3_URL}/${C2_id}`)
// 属性接口
export const reqAttr = (C3_id: string | number) =>
  request.get<any, AttrResponseData>(`${API.ATTR_URL}/${C3_id}`)
// 修改或新增属性
export const reqUpdateOrAddAttr = (data: Attr) =>
  request.post<any, string | any>(API.ADDorUPDATE_URL, data)
// 删除某个属性
export const reqDeleteAttr = (id: string) =>
